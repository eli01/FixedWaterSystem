...
//------做一个内部类--头--------------
public class EmpPane extends JPanel implements ActionListener,MouseListener{
	...
	JTable jtable;
	ProductModel pm;
	String sqlJP3="select HID 员工编号,Hname 名字,Phone 手机号,sex 性别,job 工作,pay 工资,jointime 加入时间,education 学历,address 地址 ,Remarks 备注 from Hydrotechnic";
	String[] orderStr=new String[10];
	String newCID;
	String cid;
	String[] oids=new String[1];
	String sqlOrder="insert into Hydrotechnic values(?,?,?,?,?,?,?,?,?,?)";//添加新送水工
	BillingModel bm=new BillingModel();
	String sqlCID="select top 1 HID from Hydrotechnic order by HID desc";
	...
	String sqlOrderby="select HID 员工编号,Hname 名字,Phone 手机号,sex 性别,job 工作,pay 工资,jointime 加入时间,education 学历,address 地址 ,Remarks 备注  from Hydrotechnic order by HID desc";
	...
	JLabel jlLogo=null;
	String[] jlabelString={"员工编号","名字","手机号","性别","工作","工资","加入时间","学历","地址","备注"};
	...
	public void initLogo(){
		...
		try {
			jpLogo_Image=ImageIO.read(new File("images/e.jpg"));//得到图片
		} catch (IOException e) {
	...
	public void initJp1(){
		...
		//到数据库中取出
		BillingModel bm=new BillingModel();
		newCID=DataFactory.getID(bm.query(sqlCID),"H",false);
		...
		jp1WestWest=new JPanel(new GridLayout(5,1,10,10));
		jp1WestCenter=new JPanel(new GridLayout(5,1,10,10));
		jp1East=new JPanel(new BorderLayout(5,5));
		jp1EastCenter=new JPanel(new GridLayout(5,1,10,10));
		jp1EastWest=new JPanel(new GridLayout(5,1,10,10));
	...
	public int initJTextField()
	{
		...
		if(rowNum==-1){
			JOptionPane.showMessageDialog(this, "请选中要编辑的员工！");
			return 0;
		}
	...
	public void mouseClicked(MouseEvent e) {
		...
		if(e.getSource()==cancel){			
			...
			Object selectValue=JOptionPane.showInputDialog(null, "确定保存这个员工吗？", "保存员工信息对话框", JOptionPane.INFORMATION_MESSAGE, null, possibleValues, possibleValues[0]);			
			//读取货物的信息
			if(selectValue.equals(possibleValues[0])){
				...
				String sql="insert into Hydrotechnic values(?,?,?,?,?,?,?,?,?,?)";
				if(isEdit){
					cInfo[cInfo.length-1]=cInfo[0];
					for(int i=1;i<cInfo.length;i++){
						cInfo[i-1]=cInfo[i];
					}					
					sql="update Hydrotechnic set Hname=?,Phone=?,sex=?,job=?,pay=?,jointime=?,education=?,address=?,Remarks=? where HID=?";
					isEdit=false;
				}
				bm.update(sql,cInfo);
			...
			if(rowNum==-1){//客户未选择一行并点击删除按钮
				...
				Object selectValue=JOptionPane.showInputDialog(null, "确定删除当前员工信息吗？", "员工删除对话框", JOptionPane.INFORMATION_MESSAGE, null, possibleValues, possibleValues[0]);			
								
				if(selectValue.equals(possibleValues[0])){
					//删除刚刚保存的客户信息
					String[] paras={jtf[0].getText().trim()};
					if(bm.update("delete from Hydrotechnic where HID=?", paras)){
						JOptionPane.showMessageDialog(this, "当前员工信息删除成功！");
					}else{
						JOptionPane.showMessageDialog(this, "删除失败，请查看员工编号是否正确！");
					}
				...
				BillingModel bm=new BillingModel();
				bm.update("delete from Hydrotechnic where HID=?", paras);
			...
			if(rowNum==-1){//客户未选择一行并点击删除按钮
				...
				Object selectValue=JOptionPane.showInputDialog(null, "确定要选择一个员工信息进行编辑吗？", "编辑对话框", JOptionPane.INFORMATION_MESSAGE, null, possibleValues, possibleValues[0]);			
								
				if(selectValue.equals(possibleValues[0])){
					//删除刚刚保存的客户信息
					JOptionPane.showMessageDialog(this, "请选中要编辑的员工信息！");
					return;
				}else{
					//提示接线员选中后再操作					
					return;
				...
				this.refreshClient("select HID 员工编号,Hname 名字,Phone 手机号,sex 性别,job 工作,pay 工资,jointime 加入时间,education 学历,address 地址 ,Remarks 备注 from Hydrotechnic where HID=?", cids);				
				isEdit=true;
			}	
			...
			String paras[]={cid};
			//2.调用函数刷新jtable
			this.refresh(sqlJP3+" where HID=?",paras);
			
			//刷新当前客户的信息
			//1.获得当前客户的id号（已完成）
			//2.到数据库中取数据
			String sqlClientInfo="select HID 送水工编号,Hname 名字,Phone 手机号,sex 性别,job 工作,pay 工资,jointime 加入时间,education 学历,address 地址 ,Remarks 备注 from Hydrotechnic where HID=?";
			this.refreshClient(sqlClientInfo, paras);
			//刷新头像
			clientName.setText(bm.query("select Hname from Hydrotechnic where HID=?",paras));					
		}
	}
