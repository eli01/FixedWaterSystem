...
//------做一个内部类--头--------------
public class ProPane extends JPanel implements ActionListener,MouseListener{
	...
	JTable jtable;
	ProductModel pm;
	String sqlJP3="select PID 商品编号,Wname 商品名称,Producttype 商品类型,SUID 供应商,Quantity 商品数量 ,Description 描述 from Products";
	String[] orderStr=new String[5];
	String newCID;
	String cid;
	String[] oids=new String[1];
	String sqlOrder="insert into Products values(?,?,?,?,?,?)";//添加新送水工
	BillingModel bm=new BillingModel();
	String sqlCID="select top 1 PID from Products order by PID desc";
	...
	String sqlOrderby="select PID 商品编号,Wname 商品名称,Producttype 商品类型,SUID 供应商,Quantity 商品数量 ,Description 描述  from Products order by PID desc";
	...
	JLabel jlLogo=null;
	String[] jlabelString={"商品编号","供应商,","商品名称","商品类型","商品数量","描述"};
	...
	public void initLogo(){
		...
		try {
			jpLogo_Image=ImageIO.read(new File("images/p.jpg"));//得到图片
		} catch (IOException e) {
	...
	public void initJp1(){
		...
		//到数据库中取出
		BillingModel bm=new BillingModel();
		newCID=DataFactory.getID(bm.query(sqlCID),"P",false);
		...
		jp1WestWest=new JPanel(new GridLayout(4,1,10,10));
		jp1WestCenter=new JPanel(new GridLayout(4,1,10,10));
		jp1East=new JPanel(new BorderLayout(5,5));
		jp1EastCenter=new JPanel(new GridLayout(4,1,10,10));
		jp1EastWest=new JPanel(new GridLayout(4,1,10,10));
		
		//jp1.setVisible(true);
		for(int i=0;i<3;i++){//添加标签和录入文本框
	...
	public void mouseClicked(MouseEvent e) {
		...
		if(e.getSource()==cancel){			
			...
			Object selectValue=JOptionPane.showInputDialog(null, "确定保存这个商品信息吗？", "保存商品信息对话框", JOptionPane.INFORMATION_MESSAGE, null, possibleValues, possibleValues[0]);			
			//读取货物的信息
			if(selectValue.equals(possibleValues[0])){
				String[] cInfo=new String[6];
				for(int i=0;i<cInfo.length;i++){
				...
				if(isEdit){
					...
					sql="update Products set Wname=?,Producttype=?,SUID=?,Quantity=?,Description=? where PID=?";
					isEdit=false;
				}
				bm.update(sql,cInfo);
				JOptionPane.showMessageDialog(this,"商品信息已保存！");
				this.refresh(sqlJP3);
			}else{
				return;
			}
			...
			if(rowNum==-1){//客户未选择一行并点击删除按钮
				...
				Object selectValue=JOptionPane.showInputDialog(null, "确定删除当前商品信息吗？", "商品删除对话框", JOptionPane.INFORMATION_MESSAGE, null, possibleValues, possibleValues[0]);			
								
				if(selectValue.equals(possibleValues[0])){
					//删除刚刚保存的客户信息
					String[] paras={jtf[0].getText().trim()};
					if(bm.update("delete from Products where PID=?", paras)){
						JOptionPane.showMessageDialog(this, "当前员商品息删除成功！");
					}else{
						JOptionPane.showMessageDialog(this, "删除失败，请查看商品编号是否正确！");
					}
					...
					//提示接线员选中后再操作
					JOptionPane.showMessageDialog(this, "请选中要删除的商品信息！");
					return;
				}						
			}else{
				String[] paras={(String)pm.getValueAt(rowNum, 0)};
				BillingModel bm=new BillingModel();
				bm.update("delete from Products where PID=?", paras);
			...
			if(rowNum==-1){//客户未选择一行并点击删除按钮
				...
				Object selectValue=JOptionPane.showInputDialog(null, "确定要选择一个商品信息进行编辑吗？", "编辑对话框", JOptionPane.INFORMATION_MESSAGE, null, possibleValues, possibleValues[0]);			
								
				if(selectValue.equals(possibleValues[0])){
					//删除刚刚保存的客户信息
					JOptionPane.showMessageDialog(this, "请选中要编辑的商品信息！");
					return;
				}else{
					//提示接线员选中后再操作					
					return;
				...
				this.refreshClient("select PID 商品编号,Wname 商品名称,Producttype 商品类型,SUID 供应商,Quantity 商品数量 ,Description 描述 from Products where PID=?", cids);				
				isEdit=true;
			}	
			...
			//1.获得当前客户的id号（已完成）
			//2.到数据库中取数据
			String sqlClientInfo="select PID 商品编号,Wname 商品名称,Producttype 商品类型,SUID 供应商,Quantity 商品数量 ,Description 描述 from Products where PID=?";
			this.refreshClient(sqlClientInfo, paras);
			//刷新头像
			clientName.setText(bm.query("select Wname from Products where PID=?",paras));					
		}
	}
