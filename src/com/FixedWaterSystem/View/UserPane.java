...
//------做一个内部类--头--------------
public class UserPane extends JPanel implements ActionListener,MouseListener{
	...
	JTable jtable;
	ProductModel pm;
	String sqlJP3="select userid 用户编号,username 用户名,userpasswd 用户密码,usergrade 用户级别,usersex 性别,userbg 个性设置,userjob 用户职位,userage 用户年龄,useradress 现居地址,useraser 设置的问题,useranswer 答案,userremind 问题提示 from users";
	String[] orderStr=new String[8];
	String newCID;
	String cid;
	String[] oids=new String[1];
	String sqlOrder="insert into users values(?,?,?,?,?,?,?,?,?,?,?,?)";//添加新用户
	...
	String sqlOrderby="select userid 用户编号,username 用户名,userpasswd 用户密码,usergrade 用户级别,usersex 性别,userbg 个性设置,userjob 用户职位,userage 用户年龄,useradress 现居地址,useranswer 设置的问题,useranswer 答案,userremind 问题提示 from users order by userid desc";
	...
	JLabel jlLogo=null;
	String[] jlabelString={"用户编号","用户名","用户密码","用户级别","性别","个性设置","用户职位","用户年龄","现居地址","设置的问题","答案","问题提"};
	...
	public UserPane(){		
		...
		jsp1.setDividerLocation(40);//指定logo的宽度																	
	...
	public void initLogo(){
		...
		try {
			jpLogo_Image=ImageIO.read(new File("images/u.jpg"));//得到图片
		} catch (IOException e) {
	...
	public void initJp1(){
		...
		//到数据库中取出
		BillingModel bm=new BillingModel();
		newCID=DataFactory.getID(bm.query(sqlCID),"U",false);
		...
		jp1WestWest=new JPanel(new GridLayout(6,1,10,10));
		jp1WestCenter=new JPanel(new GridLayout(6,1,10,10));
		jp1East=new JPanel(new BorderLayout(5,5));
		jp1EastCenter=new JPanel(new GridLayout(6,1,10,10));
		jp1EastWest=new JPanel(new GridLayout(6,1,10,10));
		
		//jp1.setVisible(true);
		for(int i=0;i<5;i++){//添加标签和录入文本框
	...
	public int initJTextField()
	{
		...
		if(rowNum==-1){
			JOptionPane.showMessageDialog(this, "请选中要编辑的用户！");
			return 0;
		}
	...
	public void mouseClicked(MouseEvent e) {
		...
		if(e.getSource()==cancel){			
			...
			Object selectValue=JOptionPane.showInputDialog(null, "确定保存这位用户吗？", "保存用户对话框", JOptionPane.INFORMATION_MESSAGE, null, possibleValues, possibleValues[0]);			
			//读取货物的信息
			if(selectValue.equals(possibleValues[0])){
				String[] cInfo=new String[12];
				for(int i=0;i<cInfo.length;i++){
				...
				if(isEdit){
					...
					sql="update users set username=?,userpasswd=?,usergade=?,usersex=?,userbg=?,userjob=?,userage=?,useradress=?, useraser=?,useranswer=?,userremind=? where userid=?";
					isEdit=false;
				}
				bm.update(sql,cInfo);
				JOptionPane.showMessageDialog(this,"用户已保存！");
				this.refresh(sqlJP3);
			}else{
				return;
			}
			...
			if(rowNum==-1){//客户未选择一行并点击删除按钮
				...
				Object selectValue=JOptionPane.showInputDialog(null, "确定删除当前用户吗？", "用户删除对话框", JOptionPane.INFORMATION_MESSAGE, null, possibleValues, possibleValues[0]);			
								
				if(selectValue.equals(possibleValues[0])){
					//删除刚刚保存的客户信息
					String[] paras={jtf[0].getText().trim()};
					if(bm.update("delete from users where userid=?", paras)){
						JOptionPane.showMessageDialog(this, "当前用户删除成功！");
					}else{
						JOptionPane.showMessageDialog(this, "删除失败，请查看用户编号是否正确！");
					}
				...
				BillingModel bm=new BillingModel();
				bm.update("delete from users where userid=?", paras);
			...
			if(rowNum==-1){//客户未选择一行并点击删除按钮
				...
				Object selectValue=JOptionPane.showInputDialog(null, "确定要选择一个用户进行编辑吗？", "编辑对话框", JOptionPane.INFORMATION_MESSAGE, null, possibleValues, possibleValues[0]);			
								
				if(selectValue.equals(possibleValues[0])){
					//删除刚刚保存的客户信息
					JOptionPane.showMessageDialog(this, "请选中要编辑的用户信息！");
					return;
				}else{
					//提示接线员选中后再操作					
					return;
				...
				this.refreshClient("select userid 用户编号,username 用户名,userpasswd 用户密码,usergrade 用户级别,userjob 用户职位,userage 用户年龄,useradress 现居地址,useranswer 设置的问题,useranswer 答案,userremind 问题提示  from users where userid=?", cids);				
				isEdit=true;
			}	
			...
			String paras[]={cid};
			//2.调用函数刷新jtable
			this.refresh(sqlJP3+" where userid=?",paras);
			
			//刷新当前客户的信息
			//1.获得当前客户的id号（已完成）
			//2.到数据库中取数据
			String sqlClientInfo="select userid 用户编号,username 用户名,userpasswd 用户密码,usergrade 用户级别,usersex 性别,userbg 个性设置,userjob 用户职位,userage 用户年龄,useradress 现居地址,useranswer 设置的问题,useranswer 答案,userremind 问题提示  from users where userid=?";
			this.refreshClient(sqlClientInfo, paras);
			//刷新头像
			clientName.setText(bm.query("select username from users where userid=?",paras));					
		}
	}
