...
//------做一个内部类--头--------------
public class ClientPane extends JPanel implements ActionListener,MouseListener{
	...
	public ClientPane(){		
		...
		jsp1.setDividerLocation((int)(40/SystemFigure.ResoFig));//指定logo的宽度																	
		...
		jp23.add(jsp5);
		jsp5.setDividerLocation((int)(30/SystemFigure.ResoFig));
	...
	public void mouseClicked(MouseEvent e) {
		...
		if(e.getSource()==cancel){			
			...
			if(rowNum==-1){
				...
				this.refreshClient("select CID 客户卡号,CName 客户名称,Phone 联系电话,Adress 联系地址,ContactPerson 联系人,JoinTime 加入时间 from client where CID=?", cids);				
				isEdit=true;
			}	
			...
			//1.获得当前客户的id号（已完成）
			//2.到数据库中取数据
			String sqlClientInfo="select CID 客户卡号,CName 客户名称,Phone 联系电话,Adress 联系地址,ContactPerson 联系人,JoinTime 加入时间 from client where CID=?";
