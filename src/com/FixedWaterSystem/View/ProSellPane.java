...
//------做一个内部类--头--------------
public class ProSellPane extends JPanel implements ActionListener,MouseListener{
	...
	JTable jtable;
	ProductModel pm;
	String sqlJP3="select SID 出货流水号,OID 订单流水号,PID 商品编号,Pname 商品名称 ,specification 商品规格,unitPrice 单价,quantity 数量,biscount 折扣,monerySum 金额,Remarks 备注  from ProductShipping";
	String[] orderStr=new String[10];
	String newCID;
	String cid;
	String[] oids=new String[1];
	String sqlOrder="insert into ProductShipping values(?,?,?,?,?,?,?,?,?,?)";//添加新出货单
	BillingModel bm=new BillingModel();
	String sqlCID="select top 1 SID from ProductShipping order by SID desc";
	...
	String sqlOrderby="select SID 出货流水号,OID 订单流水号,PID 商品编号,Pname 商品编号 ,specification 商品规格,unitPrice 单价,quantity 数量,biscount 折扣,monerySum 金额,Remarks 备注  from  ProductShipping order by SID desc";
	...
	JLabel jlLogo=null;
	String[] jlabelString={"出货流水号","订单流水号","商品编号","商品编号","商品规格","单价","数量","折扣","金额","备注"};
	...
	public void initLogo(){
		...
		try {
			jpLogo_Image=ImageIO.read(new File("images/ps.jpg"));//得到图片
		} catch (IOException e) {
	...
	public void initJp1(){
		...
		//到数据库中取出
		BillingModel bm=new BillingModel();
		newCID=DataFactory.getID(bm.query(sqlCID),"S",false);
		jtf[0].setText(newCID);//精确时间、H打头
	...
	public void mouseClicked(MouseEvent e) {
		...
		if(e.getSource()==cancel){			
			...
			Object selectValue=JOptionPane.showInputDialog(null, "确定保存这个出货信息吗？", "保存出货信息对话框", JOptionPane.INFORMATION_MESSAGE, null, possibleValues, possibleValues[0]);			
			//读取货物的信息
			if(selectValue.equals(possibleValues[0])){
				String[] cInfo=new String[10];
				for(int i=0;i<cInfo.length;i++){
					cInfo[i]=jtf[i].getText().trim();
				}
				String sql="insert into ProductShipping values(?,?,?,?,?,?,?,?,?,?)";
				if(isEdit){
					cInfo[cInfo.length-1]=cInfo[0];
					for(int i=1;i<cInfo.length;i++){
						cInfo[i-1]=cInfo[i];
					}					
					sql="update ProductShipping set OID=?,PID=?,Pname=?,specification=?,unitPrice=?,quantity=?,biscount=?,monerySum=?,Remarks=? where SID=?";
					isEdit=false;
				}
				bm.update(sql,cInfo);
				JOptionPane.showMessageDialog(this,"出货信息已保存！");
				this.refresh(sqlJP3);
			}else{
				return;
			}
			...
			if(rowNum==-1){//客户未选择一行并点击删除按钮
				...
				Object selectValue=JOptionPane.showInputDialog(null, "确定删除当前出货信息吗？", "出货信息删除对话框", JOptionPane.INFORMATION_MESSAGE, null, possibleValues, possibleValues[0]);			
								
				if(selectValue.equals(possibleValues[0])){
					//删除刚刚保存的客户信息
					String[] paras={jtf[0].getText().trim()};
					if(bm.update("delete from ProductShipping where SID=?", paras)){
						JOptionPane.showMessageDialog(this, "当前出货信息删除成功！");
					}else{
						JOptionPane.showMessageDialog(this, "删除失败，请查看出货流水号是否正确！");
					}
					...
					//提示接线员选中后再操作
					JOptionPane.showMessageDialog(this, "请选中要删除的出货信息！");
					return;
				}						
			}else{
				String[] paras={(String)pm.getValueAt(rowNum, 0)};
				BillingModel bm=new BillingModel();
				bm.update("delete from ProductShipping where SID=?", paras);
			...
			if(rowNum==-1){//客户未选择一行并点击删除按钮
				...
				Object selectValue=JOptionPane.showInputDialog(null, "确定要选择一个出货信息进行编辑吗？", "编辑对话框", JOptionPane.INFORMATION_MESSAGE, null, possibleValues, possibleValues[0]);			
								
				if(selectValue.equals(possibleValues[0])){
					//删除刚刚保存的客户信息
					JOptionPane.showMessageDialog(this, "请选中要编辑的出货信息！");
					return;
				}else{
					//提示接线员选中后再操作					
					return;
				...
				this.refreshClient("select SID 出货流水号,OID 订单流水号,PID 商品编号,Pname 商品名称 ,specification 商品规格,unitPrice 单价,quantity 数量,biscount 折扣,monerySum 金额,Remarks 备注  from ProductShipping where SID=?", cids);				
				isEdit=true;
			}	
			...
			String paras[]={cid};
			//2.调用函数刷新jtable
			this.refresh(sqlJP3+" where PID=?",paras);
			
			//刷新当前客户的信息
			//1.获得当前客户的id号（已完成）
			//2.到数据库中取数据
			String sqlClientInfo="select SID 出货流水号,OID 订单流水号,PID 商品编号,Pname 商品名称 ,specification 商品规格,unitPrice 单价,quantity 数量,biscount 折扣,monerySum 金额,Remarks 备注  from ProductShipping where SID=?";
			this.refreshClient(sqlClientInfo, paras);
			//刷新头像
			clientName.setText(bm.query("select Pname from Products where SID =?",paras));					
		}
	}
