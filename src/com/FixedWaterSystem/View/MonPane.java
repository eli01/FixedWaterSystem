...
//------做一个内部类--头--------------
public class MonPane extends JPanel implements ActionListener,MouseListener{
	...
	String sqlOrderby="select SID 出货流水号,OID 订单流水号,PID 商品编号,Pname 商品名称 ,specification 商品规格,unitPrice 单价,quantity 数量,biscount 折扣,monerySum 金额,Remarks 备注  from ProductShipping order by PID desc";
	...
	JLabel jlLogo=null;
	String[] jlabelString={"商品编号","商品名称","商品类型","商品数量","描述"};
		...
		if(e.getSource()==cancel){			
			...
			if(selectValue.equals(possibleValues[0])){
				...
				if(isEdit){
					...
					sql="update Products set Wname=?,Producttype=?,Quantity=?,Description=? where PID=?";
					isEdit=false;
				}
				bm.update(sql,cInfo);
			...
			if(rowNum==-1){//客户未选择一行并点击删除按钮
				...
				this.refreshClient("select PID 商品编号,Wname 商品名称,Producttype 商品类型,Quantity 商品数量 ,Descriptio 描述 from Products where PID=?", cids);				
				isEdit=true;
			}	
			...
			//1.获得当前客户的id号（已完成）
			//2.到数据库中取数据
			String sqlClientInfo="select PID 商品编号,Wname 商品名称,Producttype 商品类型,Quantity 商品数量 ,Descriptio 描述 from Products where PID=?";
