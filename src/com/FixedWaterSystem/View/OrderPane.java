...
import com.FixedWaterSystem.Tools.SystemFigure;
...
public class OrderPane extends JPanel implements ActionListener,MouseListener{
	...
	public void initSearch(){
		...
		//设置搜索组件的位置和大小
		jlRLogo.setBounds((int)(50/SystemFigure.ResoFig), (int)(5/SystemFigure.ResoFig), (int)(120/SystemFigure.ResoFig), (int)(30/SystemFigure.ResoFig));
		jtfMob.setBounds((int)(30/SystemFigure.ResoFig), (int)(50/SystemFigure.ResoFig), (int)(120/SystemFigure.ResoFig),(int)(20/SystemFigure.ResoFig));
		...
		jlMob.setBounds((int)(160/SystemFigure.ResoFig), (int)(50/SystemFigure.ResoFig), (int)(100/SystemFigure.ResoFig), (int)(20/SystemFigure.ResoFig));
		...
		jspRight1.setDividerLocation((int)(150/SystemFigure.ResoFig));
	...
	public void createPBilling(){
		...
		Vector<Vector> v=OrderPane.this.pm.rows;
		...
		for(int i=0;i<OrderPane.this.pm.getRowCount();i++){
			for(int j=0;j<OrderPane.this.pm.getColumnCount();j++){
	...
	public class ProductEditor extends JDialog implements ActionListener{
		...
		{
			rowNum=OrderPane.this.jtable.getSelectedRow();//获得选中的行
			if(rowNum==-1){
			...
			for(int i=0;i<this.columnNum;i++){
				this.jtf[i].setText(OrderPane.this.pm.getValueAt(rowNum, i).toString());
			}
		...
		public void actionPerformed(ActionEvent e) {
			...
			if(e.getSource()==confirm){//如果在Dialog中点击了确认按钮
				OrderPane.this.pm=new ProductModel();//内部类访问外部类成员
				if(this.isJtf){//修改																						
					...
					editorInfo[editorInfo.length-1]=OrderPane.this.newOID;
				}else{//添加(这个区域代码暂时还用不上)
					
				}
				if(OrderPane.this.pm.update(this.sql, editorInfo)){//在这里完成操作
					JOptionPane.showMessageDialog(this,"完成！");
				}else{
			...
			String[] cid2={OrderPane.this.cid};
			OrderPane.this.refresh(OrderPane.this.sqlJP3,cid2);//在内部类中访问外围类
		}
